...
def strip_version(source):
    return re.sub(r'^\s*#version\s+.+$', '', source, count=1,
                  flags=re.MULTILINE)
...
def discover_effects(effects_dir):
    if not os.path.isdir(effects_dir):
        return [{'id': 'default', 'name': 'Framebuffer'}]
    ...
    for filename in sorted(os.listdir(effects_dir)):
        if filename.endswith('.frag'):
            ...
            effects.append({
                'id': effect_id,
                'name': ('Framebuffer' if effect_id == 'default'
                         else effect_name(effect_id)),
            })
    if not any(effect['id'] == 'default' for effect in effects):
        effects.insert(0, {'id': 'default', 'name': 'Framebuffer'})
    return effects
