...
void mainLed(out vec4 ledColor, in vec3 ledPosition, in float ledIndex, in int sourceMode)
{
    ...
    float group = floor(ledIndex / 3.0);
    float lane = mod(ledIndex, 3.0);
    float groupPhase = hash11(group * 23.23) * 3.0;
    float tick = floor(iTime * 14.0 + groupPhase);
    float selected = step(0.965, hash11(group * 19.19 + floor(tick / 3.0) * 5.37));
